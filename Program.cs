...
namespace BlockThemAll
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (oAuth.User.Token != null)
            {
                ...
                if (!string.IsNullOrEmpty(myId))
                {
                    ...
                    result = JsonConvert.DeserializeObject<UserIdsObject>(getMyFriends(myId, "-1"));
                    while (result != null)
                    {
                ...
                Console.WriteLine(string.Format("Whitelist = {0}, Blocklist = {1}", whitelist.Count, blocklist.Count));
                ...
                while (true)
                {
                    ...
                    if (File.Exists(input.Replace("\"", "")))
                        targets = File.ReadAllText(input.Replace("\"", "")).Split(new string[] { ",", "\r\n", "\n" }, StringSplitOptions.RemoveEmptyEntries);
                    else
                        targets = input.Split(',');
                    ...
                    foreach (string target in targets)
                    {
                        ...
                        if (target.StartsWith("@"))
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(json))
                            {
                                ...
                                while (true)
                                {
                                    ...
                                    if (result.next_cursor != 0)
                                        result = JsonConvert.DeserializeObject<UserIdsObject>(getFollowers(username, result.next_cursor_str));
                                    else
                                        break;
                                }
                        ...
                        {
                            string json = searchPhase(System.Uri.EscapeDataString(target), true);
                            if (!string.IsNullOrWhiteSpace(json))
                            {
                                ...
                                while(result.search_metadata.count > 0)
                                {
                                    ...
                                    result = JsonConvert.DeserializeObject<SearchResultObject>(searchPhase(result.search_metadata.next_results, false));
                        ...
                        foreach (string s in targetLists)
                        {
                            if ((!whitelist.Contains(s)) && (!blocklist.Contains(s)))
                            {
                                Block(s);
                                blocklist.Add(s);
                                Console.WriteLine(string.Format("Current Blocklist = {0}", blocklist.Count));
                            }
                        }
                    ...
                    if (System.Console.ReadLine().ToUpper().Trim().Equals("N"))
                        break;
                }
            }
        ...
        public static string getMyFriends(string id, string cursor)
        {
            ...
            try
            {
                var req = oAuth.MakeRequest("GET", "https://api.twitter.com/1.1/friends/ids.json?stringify_ids=true&cursor=" + cursor + "&user_id=" + id + "&count=5000");
        ...
        public static string getMyFollowers(string id, string cursor)
        {
            ...
            try
            {
                var req = oAuth.MakeRequest("GET", "https://api.twitter.com/1.1/followers/ids.json?stringify_ids=true&cursor=" + cursor + "&user_id=" + id + "&count=5000");
        ...
        public static string getFollowers(string username, string cursor)
        {
            ...
            try
            {
                var req = oAuth.MakeRequest("GET", "https://api.twitter.com/1.1/followers/ids.json?stringify_ids=true&cursor=" + cursor + "&screen_name=" + username +"&count=5000");
        ...
        public static string searchPhase(string phase, bool newReq)
        {
            ...
            try
            {
                var req = oAuth.MakeRequest("GET", 
                    newReq ? "https://api.twitter.com/1.1/search/tweets.json?q=" + phase + "&result_type=recent&count=100&include_entities=false"
                           : "https://api.twitter.com/1.1/search/tweets.json" + phase);
        ...
        public static void Block(string id, bool isScreenName = false)
        {
            ...
            try
            {
                ...
                var req = oAuth.MakeRequest("POST", "https://api.twitter.com/1.1/blocks/create.json", obj);
